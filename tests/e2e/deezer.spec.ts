...
test('an operator registers a song from Deezer into an empty Station, then links one typed by hand', async ({
  ...
  // Whole-branch review. try/finally: the whole body below can throw on any
  // assertion, and a throw used to skip the `ownerContext.close()` that sat
  // after the last one, unconditionally, on the happy path only. Under
  // workers: 1 (the local setting), this context outlives the test and
  // remains open for subsequent tests when that happens. When a later test
  // fails, Playwright's recorded error-context.md then captures a page
  // snapshot from the LEAKED context instead of the failing test's own page —
  // a silent corruption that derails failure investigation, and precisely the
  // symptom this commit exists to remove. `finally` runs the close on every
  // exit, not only the one that reaches the last line.
  try {
    ...
    await expect(
      ownerPage.getByText(`The artist “${FIXTURE_ARTIST}”`, { exact: false }),
    ).toBeVisible();
