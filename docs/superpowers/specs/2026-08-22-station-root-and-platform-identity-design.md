...
| **P5b** | **Staffing a Station.** `users.manage` descends on the two doors that already take a company — `assign_company_role` and `remove_company_access` — and `users.invite` descends to the Stations an invitation names. `roles.manage` does NOT: it moves with the `roles` table in P7. `change_org_role` and `remove_member` stay, because they act on the Organization. And **`create_invitation` stops refusing an address that already has an account** — under D17 a user belongs to the platform and may be staff anywhere, so inviting an existing account is an ordinary operation that should grant access rather than raise. | Measured smaller twice. `invitation_companies` already exists (Block 1c), so an invitation already names its Stations. And the blindness D17 asks for is not a message to hide: today the door raises *"this e-mail already has an account on the platform"*, which tells any inviter that an address exists platform-wide. Making the operation succeed removes the differential instead of masking it. |
