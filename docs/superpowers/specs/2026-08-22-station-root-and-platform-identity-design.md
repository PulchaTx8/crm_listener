...
| **P5a** | **The ownership axis.** `is_owner_of_company` / `_for` stop resolving upward to the Organization and read `company_memberships`; the six Organization-level permissions descend; `has_permission_for` absorbs D19's computed Organization read grant; the `permissions` catalogue gains the read/write mark that makes the grant safe. Includes the ~20 application files that read those helpers, so the product still works at the end. | The load-bearing piece, and it must precede P6 — the listener policies P6 rewrites also test ownership — and P7, which takes away the argument the old helper reads. |
| **P5b** | **Roles and invitations descend.** Role definitions become Station-owned, seeded from platform templates; `invitations` becomes Station-scoped and answers identically for an address new to the platform and one already on it. | Needs P5a's ownership meaning settled: who may create a role is the question P5a answers. |
