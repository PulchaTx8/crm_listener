...
| **P5b** | **Staffing a Station.** `users.manage` descends on the two doors that already take a company — `assign_company_role` and `remove_company_access`. `users.invite` descends to the Stations an invitation names, on `create_invitation` and `revoke_invitation`. `roles.manage` does NOT: it moves with the `roles` table in P7. `change_org_role` and `remove_member` stay, because they act on the Organization. | Shipped. A Station's owner staffs the Station they own, and cannot reach a sister. |
| **P5b′** | **The invitation stops naming who exists.** `create_invitation` refuses an address that already has an account, in those words — the existence D17 forbids. Closing it requires **acceptance to branch**: create an account when there is none, require signing in when there is. | **Not a deletion.** That refusal is simultaneously the guard that keeps `src/services/invitations.ts` from setting a password on an existing account from an emailed link, so removing it alone would trade a disclosure for account takeover. An application change with a security property, and its own block. `78_staffing_a_station` pins the refusal so the work announces itself. |
| **P5b′** | **Role definitions descend** to the Station, seeded from platform templates. | **Deferred into P7 deliberately.** `roles` is one of the seven Organization-only tables, so the tenancy inversion has to rewrite it anyway; descending it here would be the same migration twice, and it is not needed for a Station's owner to staff their Station — only role *assignment* is per Station, and it already is. |
...
| **P7** | **Tenancy inversion.** Station becomes the root; the 43 tables drop `organization_id`; the 92 composite keys go; `companies.organization_id` becomes optional; and **`roles` descends here** (P5b′), since this migration rewrites the seven Organization-only tables regardless. | Deliberately after visibility and ownership, so only one axis is in motion at a time. |
...
**P1 and P2 shipped** (PRs #97, #98). **P0 and P5a are free to start.** P5a depends on
none of the identity work, only on `company_memberships`, which `0007`, `0013`,
`0017` and `0018` already populate. Every §6 item is settled.
