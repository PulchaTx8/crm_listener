...
**D1 — Every live promotion of the Station appears.** In its period, not
cancelled, not deleted. The same test the WhatsApp bot applies, so a promotion
configured once works at both doors with nobody ticking a box. No "accepts web"
column.
...
broken), the two doors, comments and grants.
...
D3, working as designed, and it will look like a bug to anybody who has not read
this. The console's widget tab should say how many live promotions are missing
their rules.
