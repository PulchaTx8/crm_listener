...
- **Age-driven erasure** is `sweep_retention` (`0131`, rewritten in `0133`):
  90 days on Meta's raw webhook payload, 180 on messages and conversations, 365
  on public contact requests, 30 on three operational tables. Periods are fixed
  for the installation, in the migration. **No business record is ever swept** —
