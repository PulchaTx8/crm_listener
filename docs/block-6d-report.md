...
| `npx playwright test --workers=4` | **27** specs. Two consecutive full runs: 24 passed / 3 failed, then 23 passed / 4 failed — never the same three or four specs twice, and `deadline.spec.ts` (this task's own) was among the failures exactly once. Every failing spec passed when re-run alone at `--workers=1` (7 re-runs in total, across both rounds). See §5.6 for what one of those reruns actually found underneath the noise. |

The isolation and e2e numbers are unchanged in scale from Task 10's own
(223 cases / 20 files; 597 unit tests was already Task 8's count and Task 10
added no unit tests — Task 11 added none either, only the one e2e spec).
`db:test` grew by nothing over Task 12's own 953/953: Task 11 touches no
migration.
...
| D6 — one poisoned winner must not stop every Station | a procedure, one `commit` per winner inside its own exception block; `12b_deadline_sweep.test.sql` proves it — see §5.7 for what that proof actually depends on |
...
### 5.4 `decodeCursor` is fixed — the count of screens sharing that hole went from six to zero
...
coming back `22P02`. Every keyset screen shares the one fix: `/participations`,
`/members`, `/promotions`, `/pickups` (new this block) and
`/inventory/movements` (new this block). Task 7 also caught and fixed four
tests in `tests/unit/keyset.test.ts` that fed non-uuid ids through the real
`decodeCursor` and were quietly asserting the hole rather than closing it.
...
### 5.8 `list_participations` takes unvalidated uuid parameters straight from the URL
...
### 5.9 The e2e suite's concurrency behaviour, confirmed again
...
- `list_participations`'s unvalidated uuid parameters (§5.8) — Block 6c's to
  fix.
