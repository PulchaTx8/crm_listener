...
-- Block 17b added WEB the same way (0166): a listener standing on the Station's
-- own website, through the embedded widget.
...
-- attending the listener on WhatsApp. WEB is a third path again, and it is not
-- API: nobody authenticated with a credential, and nothing was posted by an
-- application. A visitor pressed a button on a page this product served.
select is(
  enum_range(null::public.music_request_channel)::text[],
  array['MANUAL', 'IMPORT', 'API', 'WEB'],
  'music_request_channel gained WEB in Block 17b; WHATSAPP is still reserved for our own bot');
