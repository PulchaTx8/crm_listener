...
-- user session is a way to take real recipient rows out of circulation:
-- calling this twice from two browser tabs would mark them `claimed` outside
-- of any send actually happening. At the time this migration was written
-- nothing gave them back at all. Task 6b then built the reclaim, as a DIRECT
-- WRITE from the drain rather than a new RPC (src/services/campaigns.ts, step
-- 1 of drainCampaigns; 0245 is the index it scans) --
-- message_campaign_recipients already grants service_role select+update for
-- exactly that shape of write (0242's own grant comment, on settling a send's
-- outcome, claimed_at among the columns it names). NOTE that the reclaim
-- gives a row back UNCONDITIONALLY, which is why the drain -- not this claim
-- -- is where a cancelled campaign's reclaimed rows are settled `cancelled`
-- instead of sent (whole-branch review C1, ruling R34): a row this function
-- refused to claim would be a row nothing ever settles.
