...
-- recipient rows with.
--
-- claim_outbox_batch (0063, amended by 0111 and 0165) is this function's
-- template, read live via pg_get_functiondef rather than trusted from any one
-- migration's text, because it has been amended twice since 0063 first wrote
-- it and the version on disk in any single file is not the version running.
...
-- partial on exactly that predicate, and claim_outbox_batch's own migration
-- (0059) explains why: a predicate naming a status this function can ALSO
-- see would stop being part of the index condition and become a FILTER
-- applied after the fetch, and whether the planner still uses the index at
-- all would then depend on how large the table had grown -- a claim that
-- silently stops using its index rather than visibly failing to.
--
...
-- outside of any send actually happening, and nothing un-claims them but a
-- reclaim this block has not built yet (0242's own comment on `claimed_at`
-- names that as a later task's job, the same role outbox_messages.claimed_at
-- plays for STALE_CLAIM).
...
comment on function public.claim_campaign_batch(integer) is
  'The next campaign recipients to send, marked claimed in the same statement that chooses them -- claim_outbox_batch''s shape (0063/0111/0165), read live via pg_get_functiondef rather than trusted from any one of those three migrations'' text. ONE STATEMENT: pg_cron-driven overlap is the ordinary case for this worker (0063''s own reasoning), and a plain select would let two ticks claim the same rows and send a listener the campaign twice -- the exact complaint that costs a WhatsApp number its quality rating, spec''s own reason this whole block exists. FOR UPDATE SKIP LOCKED so an overlapping call takes the next rows instead of blocking on these. Scans message_campaign_recipients_sendable_idx (0242), partial on `status = ''pending''` alone -- naming `claimed` here too would turn that predicate into a filter applied after the fetch and make the planner''s use of the index depend on table size, the same warning claim_outbox_batch''s own migration (0059) states for itself. Returns attempts UNCHANGED, because claiming is not attempting. LEFT JOINs message_campaigns and message_templates for a reason that is structural rather than "these might be missing": the claim CTE''s UPDATE runs unconditionally as a data-modifying WITH query, so an INNER JOIN that failed to match would silently drop an already-claimed row from what this function returns while leaving it claimed in the table. variables comes back exactly as message_campaign_recipients stores it -- positional, snapshot-time, never re-ordered against message_templates.variables'' current order (0242''s own warning). Does not return from_name, from_email or reply_to (0223): the drain resolves sender identity once per campaign, not once per row, so widening this claim would be the wrong fix for that. GRANTED TO service_role ALONE: the drain (Task 6) is the only caller, and a claim reachable by a user session is a way to take real recipient rows out of circulation that nothing in this block can yet give back -- the reclaim is a later task''s job, the same role reclaim_stale_whatsapp_claims (0063) plays for outbox_messages, and message_campaign_recipients.claimed_at''s own comment (0242) already says so.';
