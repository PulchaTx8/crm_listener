...
/** This route's own kind. labels/page.tsx is this file with KIND and copy swapped — D2's "thin page" half. */
const KIND = 'GENRE' as const;
...
export default async function GenresPage({
  ...
  // archive_music_reference and music_genres' own select policy (0099) each
  ...
  try {
    ...
    logger.error({ err: cause, companyId: selected.id }, 'could not load the genres list');
    return <LoadError message={describeMusicReadError(cause, t)} />;
  }

  const copy: ReferenceScreenCopy = {
    title: t('genres'),
    description: t('referenceGenresDescription'),
    createButton: t('registerGenre'),
    createDialogTitle: t('registerAGenre'),
    archiveButton: t('archiveGenre'),
    archiveConfirmTitle: t('archiveThisGenreQuestion'),
    readOnlyNotice: t('youDoNotHoldMusicManageForThisRecord'),
    emptyMessage: t('noGenresAreRegisteredInThis'),
    noMatchMessage: t('noGenreMatchesTheseFilters'),
    countLabel: t('genresCountLabel', { count: page.total }),
    searchPlaceholder: t('genreName'),
    searchAriaLabel: t('searchGenresByName'),
  };
