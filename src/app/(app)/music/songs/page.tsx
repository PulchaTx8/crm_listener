...
export default async function SongsPage({
  ...
  let songwriters: ReferenceSummary[];
  let page: SongListPage;
  let permissions: MusicPermissions;
  try {
    [artists, labels, genres, songwriters, albums, page, permissions] = await Promise.all([
      ...
      // which is the whole point of making a songwriter the fifth
      // music_reference_kind: it is a name and a legacy id, exactly like the
      // three above it.
      listMusicReferences(selected.id, 'SONGWRITER'),
      ...
      listSongsPage({
        ...
        songwriterId: state.songwriterId,
        sort: state.sort,
  ...
  return (
    <>
      ...
      <SongsFilters state={state} artists={artists} genres={genres} songwriters={songwriters} />
