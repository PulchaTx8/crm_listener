...
export interface SongListState {
  ...
  genreId?: string;
  /** A songwriter id; undefined means every songwriter. */
  songwriterId?: string;
  sort: SongSortKey;
...
export function parseSongListState(raw: MusicSearchParams, companyId: string): SongListState {
  ...
  return {
    ...
    songwriterId: raw.songwriter?.trim() || undefined,
    sort,
    direction,
  };
}
...
export function hasActiveSongFilters(state: SongListState): boolean {
  return Boolean(state.search || state.artistId || state.genreId || state.songwriterId);
}
...
export function songHref(state: SongListState, cursor?: SongCursor | null): string {
  ...
  if (state.songwriterId) query.set('songwriter', state.songwriterId);
